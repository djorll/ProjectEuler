...
# incorrectly believe that 49/98 = 4/8,
...
def gcd(a, b):
    # gcd retourne le plus grand commun diviseur (greatest common divisor) de 2 entiers donnés.
    while b:
        a, b = b, a % b
    return a
        ...
        if i / j < 1 and i != j:
            for x in range(1, 10):
                if x != i and x != j:
                    ...
                    den1 = "{}{}".format(j, x)
                    den2 = "{}{}".format(x, j)
                    if int(num1) / int(den1) == i / j or int(num1) / int(den2) == i / j or int(num2) / int(
                            den1) == i / j or int(num2) / int(den2) == i / j:
                        numerateur *= i
                        denominateur *= j
